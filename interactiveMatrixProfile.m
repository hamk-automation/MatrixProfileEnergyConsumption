...
radius = 2.0;
updatePeriod = 0.1; % in second
anytimeMode = 1; % 1: original with mass O(n^2 log n); 
                 % 2: SCRIMP
                 % 3: SCRIMP++
...
%% check input
dataLen = length(data);
% annoLen = length(annotationVector);
% 
% if annoLen == size(annotationVector,2) 
...
%         excZoneLen
    ...
    if exist('annotationVector', 'var')
        ...
        if i == 1
          annotationVector = annotationVector(dataLen - proLen +1:end);
%             annotationVector = annotationVector(1:dataLen -subLen+1);
        end
