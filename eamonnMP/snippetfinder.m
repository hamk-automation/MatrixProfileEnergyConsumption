...
%     plot(distance);title(['MPdist-' num2str(n) '  location-' num2str(indexes(index))]);box off; xlim([0 length(distance)])
    ...
    if N == 2
        ...
        box off;xlim([0 length(data(indexes(index):indexes(index)+sub))]);0:1:length(data(indexes(index):indexes(index)+sub));
    else
        plot(data(indexes(index):indexes(index)+sub),'color',CM(n,:));title([ ' snippet-' num2str(n) '  location-'  num2str(indexes(index))],'color',CM(n,:))
        box off;xlim([0 length(data(indexes(index):indexes(index)+sub))]);xticks(0:1:length(data(indexes(index):indexes(index)+sub)));
    end
